export const name = "javascript/react-native";

export function formatter(dictionary: { allProperties: any[] }) {
  let hasSizeToken = dictionary.allProperties.some(
    (prop) => prop.attributes.category == "size"
  );
  ...
  let to_ret = dictionary.allProperties
    .map(function (prop) {
      ...
      if (prop.comment) to_ret = to_ret.concat(" // " + prop.comment);
  ...
  return (
    (hasSizeToken ? "import { PixelRatio } from 'react-native';\n\n" : "") +
    to_ret
  );
}
